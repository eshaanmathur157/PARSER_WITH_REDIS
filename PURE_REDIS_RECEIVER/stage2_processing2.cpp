...
void process_transactions_parallel(
    ...
    std::mutex& hot_address_mutex
) {
    // 🔒 ACQUIRE LOCK, COPY HOT ADDRESSES, RELEASE LOCK
    HotAddressLookups local_hot_addresses;
    {
        std::lock_guard<std::mutex> lock(hot_address_mutex);
        std::cout << "[Process Parallel] 🔒 Mutex locked - copying hot addresses..." << std::endl;
        
        // --- MANUAL DEEP COPY (Skips the Mutex) ---
        // 1. Copy the raw strings
        local_hot_addresses.storage = hot_addresses.storage;
        
        // 2. Copy the set of hashes
        local_hot_addresses.pool_hashes = hot_addresses.pool_hashes;

        // 3. REBUILD THE MAP 
        // We must rebuild this because string_views must point to the NEW 'local_hot_addresses.storage'
        // If we just copied the map, it would point to the global storage, which might move in memory!
        local_hot_addresses.hash_to_addr_map.reserve(local_hot_addresses.storage.size());
        
        for (const std::string& addr_str : local_hot_addresses.storage) {
            // Re-calculate hash or lookup is cheap enough, but safe way is to link view to new storage
            // Optimally, we assume hashes match storage order, but re-hashing ensures safety
            uint64_t hash = XXH3_64bits_withSeed(addr_str.data(), addr_str.length(), 0);
            local_hot_addresses.hash_to_addr_map[hash] = addr_str; 
        }

        std::cout << "[Process Parallel] ✅ Hot addresses copied - releasing lock" << std::endl;
    } // 🔓 Mutex auto-unlocked here
    
    std::cout << "[Process Parallel] 🔓 Mutex unlocked - proceeding with local copy" << std::endl;
    ...
    const std::string data_center_index = "-2";
    ...
    auto worker_fn = [&](size_t thread_id) {
        ...
        auto client_result = arrow::flight::FlightClient::Connect(*location_result, arrow::flight::FlightClientOptions());
        if (!client_result.ok()) {
        ...
        for (size_t tx_idx = start_tx; tx_idx < end_tx; ++tx_idx) {
            ...
            // 🔥 USE LOCAL_HOT_ADDRESSES 
            bool is_pool_tx =
               check_array_for_hashes(tx_views.accountKeys_view, local_hot_addresses.pool_hashes, address_strings) ||
               check_array_for_hashes(tx_views.writable_view, local_hot_addresses.pool_hashes, address_strings) ||
               check_array_for_hashes(tx_views.readonly_view, local_hot_addresses.pool_hashes, address_strings);
            ...
            for (const auto& addr : address_strings) {
                ...
                // 🔥 USE local_hot_addresses
                if (local_hot_addresses.pool_hashes.count(hash)) 
                    index_to_hot_addr_map[current_index] = local_hot_addresses.hash_to_addr_map.at(hash);
                current_index++;
            }
        ...
        if (!status.ok()) {
            std::cerr << "Thread " << thread_id << " DoneWriting Error: " << status.ToString() << std::endl;
        }

        status = writer->Close();
        if (!status.ok()) {
            std::cerr << "Thread " << thread_id << " Close Error: " << status.ToString() << std::endl;
        }
    };
